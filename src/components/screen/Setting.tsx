...
const SettingListItem = ({ item }) => <StyledSettingListItemView>
  <StyledContentView>
...
const Setting: React.FC<SettingProps> = ({ theme }) => {
  ...
  const [isNightMode, setIsNightMode] = useState(state.theme === ThemeType.NIGHT);
  ...
  useEffect(() => {
    if (isNightMode) {
      dispatch({ type: 'change-theme-mode', payload: { theme: ThemeType.NIGHT } });
    } else {
      dispatch({ type: 'change-theme-mode', payload: { theme: ThemeType.LIGHT } });
    }
  }, [isNightMode]);
